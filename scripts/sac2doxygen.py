...
def clean_line (line):
    ...
    if re.search (symbfuncs, line):
        print ('/** @note The name of the function has been altered for documentation reasons, **make sure to use the appropriate symbol instead**, e.g. _PLUS_symbol_ -> _+_ */')
        # we have to apply these throughout, as some functions might appear all on one line
        line = re.sub (r'\s\+\s*\(', r' PLUS_symbol (', line)
        line = re.sub (r'\s\+\+\s*\(', r' INC_symbol (', line)
        line = re.sub (r'\s\-\s*\(', r' MINUS_symbol (', line)
        line = re.sub (r'\s\-\-\s*\(', r' DEC_symbol (', line)
        line = re.sub (r'\s\/\s*\(', r' DIV_symbol (', line)
        line = re.sub (r'\s\*\s*\(', r' MULT_symbol (', line)
        line = re.sub (r'\s\%\s*\(', r' MOD_symbol (', line)
        line = re.sub (r'\s<\s*\(', r' LT_symbol (', line)
        line = re.sub (r'\s<=\s*\(', r' LE_symbol (', line)
        line = re.sub (r'\s>\s*\(', r' GT_symbol (', line)
        line = re.sub (r'\s>=\s*\(', r' GE_symbol (', line)
        line = re.sub (r'\s==\s*\(', r' EQ_symbol (', line)
        line = re.sub (r'\s!=\s*\(', r' NE_symbol (', line)
        line = re.sub (r'\s\|\|\s*\(', r' OR_symbol (', line)
        line = re.sub (r'\s&&\s*\(', r' AND_symbol (', line)
        line = re.sub (r'\s!\s*\(', r' NOT_symbol (', line)
        line = re.sub (r'\s&\s*\(', r' BIT_AND_symbol (', line)
        line = re.sub (r'\s<<\s*\(', r' BIT_LSHIFT_symbol (', line)
        line = re.sub (r'\s>>\s*\(', r' BIT_RSHIFT_symbol (', line)
        line = re.sub (r'\s\|\s*\(', r' BIT_OR_symbol (', line)
        line = re.sub (r'\s^\s*\(', r' BIT_XOR_symbol (', line)
        line = re.sub (r'\s~\s*\(', r' BIT_NOT_symbol (', line)
