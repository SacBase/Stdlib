...
void SACstrmod(string str, sac_int pos, unsigned char c)
{
    ...
    if (strlength <=(size_t)pos)
        SAC_RuntimeError("strmod: pos("PRIisac") outside of string(length %zu)",
                         pos, strlength);
...
void SACstrovwt(string outer, sac_int pos, string inner)
{
    ...
    if ((size_t)pos+inner_length > outer_length) {
        SAC_RuntimeError("strovwt: Overwriting string ends at position "PRIisac
                         " while string is of length %zu",
                         pos + inner_length, outer_length);
    }
...
unsigned char SACstrsel(string str, sac_int pos)
{
    ...
    if ((size_t)pos >= strlength)
        SAC_RuntimeError("strsel: pos("PRIisac") outside of string(length %zu)",
                         pos, strlength);
...
void SACstrtake(string str, sac_int pos)
{
    ...
    if ((size_t)pos >= strlength) {
        SAC_RuntimeError("strtake: pos("PRIisac") outside of string(length %zu)",
                         pos, strlength);
    }
...
string SACstrext(string str, sac_int pos, sac_int len)
{
    ...
    if ((size_t)(pos + len) >= strlength) {
        SAC_RuntimeError("strext: Selecting substring ends at position "PRIisac
                         " while string is of length %zu",
                         pos + len, strlength);
    }
