 ...
 * monotonic time clock into the world of SAC.
 *
 ******************************************************************************/
#include <time.h>
#ifdef __MACH__
...
#else
    /* Using the raw monotonic clock because the latter is NOT influenced by
     * changes to system time (be it via an admin changing the time of the
     * system or NTP updating the time).
     * If however the raw monotonic clock is not available, we default to using
     * the regular monotonic clock - which can be influenced by NTP and the
     * like. */
